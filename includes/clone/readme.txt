...
* pspin_duplicate_post_get_original template tag
...
I have added the template tag `pspin_duplicate_post_clone_post_link( $link, $before, $after, $id )`, which behaves just like [edit_post_link()](http://codex.wordpress.org/Function_Reference/edit_post_link).
...
Another available template tag is `pspin_duplicate_post_get_original($id, $output)` which returns the original post, either as a post object, an associative array or a numeric array (depending on the $output parameter), jus as [get_post()](http://codex.wordpress.org/Function_Reference/get_post) does.
`pspin_duplicate_post_get_original()` relies on the `_dp_original` custom field.
